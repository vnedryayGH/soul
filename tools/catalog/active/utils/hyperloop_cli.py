...
import gzip
import hashlib
# Ensure project root is importable so that 'tools.catalog.*' can be resolved when running as a script
try:
...
def _print_raw_json(data: object) -> None:
    """Print JSON minified to stdout only (no extra logs), UTF-8 safe.

    Handles Windows consoles with legacy codepages (e.g., cp1251) by:
    1) Normal write with ensure_ascii=False
    2) On UnicodeEncodeError → try reconfigure stdout to UTF-8
    3) Fallback: write UTF-8 bytes to stdout.buffer
    4) Last resort: ensure_ascii=True (ASCII-only)
    """
    ...
    try:
        ...
        try:
            # Python 3.7+: reconfigure stdout on the fly
            sys.stdout.reconfigure(encoding="utf-8", errors="replace")  # type: ignore[attr-defined]
            sys.stdout.write(payload)
        except Exception:
            try:
                # As a last binary-safe resort, write UTF-8 bytes
                sys.stdout.buffer.write(payload.encode("utf-8", errors="replace"))  # type: ignore[attr-defined]
            except Exception:
                # Final fallback: ASCII-only to avoid crashes
...
def _print_raw_text(text: str) -> None:
    try:
        ...
        try:
            ...
            try:
                sys.stdout.buffer.write(text.encode("utf-8", errors="replace"))  # type: ignore[attr-defined]
            except Exception:
                # Best-effort ASCII fallback
                sys.stdout.write(text.encode("ascii", errors="replace").decode("ascii", errors="replace"))
    try:
        sys.stdout.flush()
    except Exception:
        pass
...
def main(argv: list[str]) -> int:
    ...
    if args.signed:
        try:
            # Prefer local utils client to avoid PYTHONPATH issues
            from tools.catalog.active.utils import SoulHyperloopClient  # type: ignore
        except Exception:
            from Soul.soul_hyperloop import SoulHyperloopClient  # type: ignore  # fallback if available
        ...
        try:
            ...
            if should_fallback:
                try:
                    ...
                    from Soul.soul_hyperloop import SoulHyperloopClient  # type: ignore
                base = args.api_url.rstrip("/")
                client = SoulHyperloopClient(base_api_url=base, telegram_user_id=args.telegram_user_id, timeout=args.timeout)
